...
check_storage() {
    ...
    if [ ! -d "/mnt/data" ]; then
        echo -e "${RED}${BOLD}[✗] Error: /mnt/data not found. Creating directory...${NC}"
        sudo mkdir -p /mnt/data || {
            echo -e "${RED}${BOLD}[✗] Failed to create /mnt/data${NC}"
            exit 1
        }
    fi
    ...
    if ! touch /mnt/data/test_write 2>/dev/null; then
        echo -e "${YELLOW}${BOLD}[!] Adjusting permissions for /mnt/data...${NC}"
        sudo chown -R $USER:$USER /mnt/data && \
        sudo chmod -R 775 /mnt/data || {
            echo -e "${RED}${BOLD}[✗] Failed to set permissions${NC}"
            exit 1
        }
    fi
    rm -f /mnt/data/test_write
    
    # Verify sufficient space (minimum 5GB)
    AVAIL_SPACE=$(df -BG /mnt/data | awk 'NR==2 {print $4}' | tr -d 'G')
    if [ "$AVAIL_SPACE" -lt 5 ]; then
        echo -e "${RED}${BOLD}[✗] Insufficient space (${AVAIL_SPACE}GB). Need at least 5GB.${NC}"
        exit 1
    fi
    
    echo -e "${GREEN}${BOLD}[✓] Storage verified: $(df -h /mnt/data | awk 'NR==2 {print $4}') available${NC}"
...
cleanup() {
    echo -e "${YELLOW}${BOLD}[✓] Cleaning up processes and resources...${NC}"
...
# Replace the Python virtual environment section with this:
VENV_PATH="/mnt/data/.venv"

echo -e "${CYAN}${BOLD}[✓] Setting up Python virtual environment on external storage...${NC}"
if [ ! -d "$VENV_PATH" ]; then
    python3 -m venv "$VENV_PATH" || {
        echo -e "${RED}${BOLD}[✗] Failed to create virtual environment. Check storage permissions.${NC}"
        exit 1
    }
fi
...
# Set PIP cache directory to external storage
...
if [ -z "$CONFIG_PATH" ]; then
    if command -v nvidia-smi &> /dev/null || [ -d "/proc/driver/nvidia" ]; then
        ...
        pip install --disable-pip-version-check --cache-dir "$PIP_CACHE_DIR" --no-cache-dir -q -r "$ROOT"/requirements-gpu.txt
        pip install flash-attn --no-build-isolation
    else
        echo -e "${YELLOW}${BOLD}[✓] No GPU detected, using CPU configuration${NC}"
        pip install --disable-pip-version-check --cache-dir "$PIP_CACHE_DIR" --no-cache-dir -q -r "$ROOT"/requirements-cpu.txt
